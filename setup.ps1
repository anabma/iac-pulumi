# ============================================================
# client Pulumi – Local Setup Script (Windows)
...
Write-Host ""
Write-Host "======================================"
Write-Host "  client Pulumi Setup"
Write-Host "======================================"
Write-Host ""
...
try {
    aws s3 mb s3://client-pulumi-state --region eu-north-1 2>$null
...
if (-not (Test-Path $keyPath)) {
    ...
    ssh-keygen -t rsa -b 4096 -f $keyPath -N '""' -C "client"
...
pulumi config set --secret client-infrastructure:db_password $DB_PASS_PLAIN --stack dev
